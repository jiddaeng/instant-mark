...
- `고쟁이 공통수학 2`와 `올림포스 유형편 공통수학 2`가 기본 답지로 들어 있습니다.
...
기본 고쟁이와 올림포스 답지는 포함된 문항 좌표를 바로 사용합니다. 추가한
PDF가 지원되는 2단 해설 형식이면 브라우저가 문항 좌표를 자동으로 만들어
빠른 채점 기능을 활성화하고, 생성된 좌표도 IndexedDB에 저장합니다.
...
- `problem-index.json`과 `olympus-index.json`에는 문항별 PDF 페이지와 자르기
  좌표만 저장합니다.
